...
public class CompanyStatistics {
	...
	public static void createCompany() {		
		...
		try {
			for(int i=0; i < 5; i++) {
				...
				person = PersonFactory.create("Manager", scanner.nextLine(), rand.nextInt(40) + 20,new Salary(rand.nextDouble()*26000 + 1000,rand.nextInt(10000)));
				if(person != null && person instanceof Manager) {
					company.departments[i]= new Department(departmentNames[i].getName(),(Manager)person);
					Employee[] employees = company.departments[i].getEmployees();
					for(int j=0; j < 15; j++) {
						System.out.println("Please enter employee " + (j+1) + " name");
						person = PersonFactory.create("Employee", scanner.nextLine(), rand.nextInt(40) + 20,new Salary(rand.nextDouble()*49000 + 1000,rand.nextInt(10000)));
	...
	public static void averageSalaryOfEachDepartment() {
		for(int i=0; i<departments.length; i++) {
			System.out.println("Department= " + departments[i].getDepartmentName() + ", and the average salary is= " + String.format("%.2f",departments[i].averageSalary()) + "NIS");	
		}
	}
	...
	public static void averageBonusOfEachDepartment() {
		for(int i=0; i<departments.length; i++) {
			System.out.println("Department= " + departments[i].getDepartmentName() + ", and the average bonus is= " + String.format("%.2f",departments[i].averageSalary()) + "NIS");	
		}
	}
	...
	public static void numOfWorkers1AndAverageSalary() {
		System.out.println("The number of employees without managers in ages 20-30= " + company.getNumberOfEmployeesInAges20To30());
		try {
			System.out.println("The average salary is= " + String.format("%.2f",company.getAverageSalaryOfEmployeesWithAgeIn20To30()) + "NIS");
		}catch(CompanyException e) {
			System.out.println(e.getMessage());
		}
	}
	...
	public static void numOfWorkers2AndAverageSalary() {
		System.out.println("The number of employees without managers in ages 31 and up= " + company.getNumberOfEmployeesInAges31Up());
		try {
			System.out.println("The average salary is= " + String.format("%.2f",company.getAverageSalaryOfEmployeesWithAge31Up()) + "NIS");
		}catch(CompanyException e) {
			System.out.println(e.getMessage());
		}
	}
